...
export default function LoansPage() {
  ...
  const canApprove =
    user?.role === "chairman" || user?.role === "board_director";
  ...
  const memberOptions =
    memberResults?.data?.data?.data ?? memberResults?.data?.data ?? [];
  ...
  const guarantorOptions = (
    guarantorResults?.data?.data?.data ??
    guarantorResults?.data?.data ??
    []
  ).filter(
    (m: any) =>
      m.id !== selectedMember?.id && !guarantors.some((g) => g.id === m.id),
  );
  ...
  const purgeEligible = purgeData?.data?.data ??
    purgeData?.data ?? { loans: [], retention_years: 2, eligible_count: 0 };
  ...
  const createMutation = useMutation({
    mutationFn: () => {
      ...
      fd.append(
        ...
        JSON.stringify(guarantors.map((g) => g.id)),
      );
    ...
    onSuccess: () => {
      toast.success(
        "Loan application submitted — awaiting Chairman/Board Director approval",
      );
    ...
    onError: (e: any) => {
      toast.error(
        e.response?.data?.message ?? "Failed to submit loan application",
      );
      setConfirmingCreate(false);
    },
  });
  ...
  const approveMutation = useMutation({
    ...
    onError: (e: any) => {
      toast.error(e.response?.data?.message ?? "Could not approve loan");
      setConfirmingApprove(null);
    },
  });
  ...
  const rejectMutation = useMutation({
    ...
    onError: (e: any) =>
      toast.error(e.response?.data?.message ?? "Could not reject loan"),
  });
  ...
  const purgeMutation = useMutation({
    ...
    onError: (e: any) =>
      toast.error(e.response?.data?.message ?? "Could not purge document"),
  });
  ...
  const canSubmitCreate =
    ...
    guarantors.length >= 1 &&
    guarantors.length <= 2 &&
    !!createForm.principal_amount &&
    !!createForm.monthly_repayment &&
    !!createForm.due_date &&
    !!documentFile;
  ...
  return (
    <div>
      <TopBar
        title="Loans"
        subtitle="Qard Hasan (interest-free) loan management"
      />
      <div className="p-6">
        <PageHeader
          title="Loan Register"
          ...
          action={
            <div className="flex gap-2">
              <button
                onClick={() => setShowPurgeQueue(true)}
          ...
          {[
            {
              ...
              value: s?.pending_approval_loans ?? 0,
              cls: "badge-yellow",
            },
            {
              label: "Active Loans",
            ...
            {
              label: "Fully Paid",
              value: s?.fully_paid_loans ?? 0,
              cls: "badge-green",
            },
            {
              label: "Defaulted",
              value: s?.defaulted_loans ?? 0,
              cls: "badge-red",
            },
          ].map(({ label, value, cls }) => (
            <div key={label} className="card p-4 text-center">
              <p
                className={`text-xl font-bold mb-1 ${cls}`}
                style={!cls ? { color: "var(--forest)" } : {}}
              >
                {value}
              </p>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
          ...
          <input
            ...
            className="w-full sm:w-64 px-3 py-2 rounded-xl border text-sm"
                    ...
                    >
                      ...
                      <td className="px-4 py-3 whitespace-nowrap">
                        {loan.status === "pending_approval" &&
                          canApprove &&
                          loan.created_by?.id !== user?.id && (
                            <div className="flex gap-2">
                              <button
                                onClick={() => setConfirmingApprove(loan)}
            ...
            <div className="md:hidden space-y-3 p-3">
              {loans.map((loan) => (
                <MobileCard key={loan.id}>
                  ...
                  <MobileRow
                    label="Submitted By"
                    value={loan.created_by?.full_name ?? "—"}
                  />
                  {loan.status === "pending_approval" &&
                    canApprove &&
                    loan.created_by?.id !== user?.id && (
      ...
      >
        <div className="space-y-4">
            ...
            {selectedMember ? (
              ...
              <div className="relative">
                <Input
                  ...
                  placeholder="Search by name or MUTAN ID…"
                />
          ...
          {/* Guarantors */}
          <FormField
            label={`Guarantor${guarantors.length !== 1 ? "s" : ""} (1 required, 2 allowed)`}
            required
            ...
            <div className="space-y-1.5 mb-2">
              {guarantors.map((g) => (
                ...
                >
                  <span>
                    {g.full_name}{" "}
                    <span className="text-gray-400">{g.mutan_id}</span>
                  </span>
                  <button
                    type="button"
                    onClick={() =>
            ...
            {guarantors.length < 2 && selectedMember && (
              <div className="relative">
                <Input
                  ...
                  placeholder="Search a guarantor by name or MUTAN ID…"
                />
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <FormField label="Monthly Repayment (₦)" required>
              <Input
                type="number"
                value={createForm.monthly_repayment}
                onChange={(e) =>
                  setCreateForm((f) => ({
                    ...f,
                    monthly_repayment: e.target.value,
                  }))
                }
                placeholder="10000"
              />
          ...
          <FormField
            label="Filled Loan Form (hardcopy scan or photo)"
            required
            hint="JPG, PNG, or PDF — max 150KB. Required before this application can be submitted for approval."
          >
            <label
              className="flex items-center gap-2 px-3 py-2.5 rounded-xl border text-xs cursor-pointer hover:bg-gray-50"
              style={{
                borderColor: documentFile ? "var(--forest)" : "#e2e8f0",
              }}
            >
              <Upload size={14} />
          ...
          >
            ...
            <p className="text-gray-600 mt-0.5">
              No interest, in compliance with Islamic financial principles. This
              submits the application only — a Chairman or Board Director must
              still approve it before anything disburses. Members may hold more
              than one active loan at a time.
            </p>
          </div>
      ...
      <ConfirmDialog
        ...
        message={
          selectedMember
            ? `This submits a ₦${Number(createForm.principal_amount || 0).toLocaleString()} loan application for ${selectedMember.full_name}, guaranteed by ${guarantors.map((g) => g.full_name).join(" and ")}. Nothing disburses until a Chairman or Board Director approves it.`
            : ""
        }
      ...
      <ConfirmDialog
        ...
        onConfirm={() =>
          confirmingApprove && approveMutation.mutate(confirmingApprove.id)
        }
        title="Approve & disburse this loan?"
        message={
          confirmingApprove
            ? `This disburses ₦${confirmingApprove.principal_amount.toLocaleString()} to ${confirmingApprove.member?.full_name} immediately. This cannot be undone from here.`
            : ""
        }
      ...
      >
        <div className="space-y-4">
          <p className="text-xs text-gray-500">
            Rejecting the ₦{rejecting?.principal_amount.toLocaleString()}{" "}
            application for {rejecting?.member?.full_name}.
          </p>
          <FormField label="Reason for rejection" required>
            <Textarea
              value={rejectReason}
              onChange={(e) => setRejectReason(e.target.value)}
              placeholder="e.g. Guarantor is not in good standing — please resubmit with an alternative guarantor."
            />
          </FormField>
          <button
            onClick={() =>
      ...
      <Modal
        open={showPurgeQueue}
        onClose={() => setShowPurgeQueue(false)}
        title="Loan Document Retention"
        size="lg"
      >
        <div className="space-y-4">
          <p className="text-xs text-gray-500">
            Fully-paid loans keep their hardcopy scan on file for{" "}
            {purgeEligible.retention_years} year(s) after the loan is cleared.
            Once that window passes, any admin can purge the document — the
            loan's financial record is never deleted, only the scan.
          </p>
          {purgeEligible.loans?.length === 0 ? (
            <EmptyState
              icon={Archive}
              title="Nothing eligible yet"
              description="Loans will appear here once fully paid and past the retention window."
            />
          ) : (
                ...
                >
                  <div>
                    ...
                    <p className="text-gray-400">
                      Principal {formatCurrency(l.principal_amount)} · fully
                      paid {formatDateTime(l.fully_paid_at)}
                    </p>
                  </div>
                  <button
                    onClick={() => purgeMutation.mutate(l.id)}
